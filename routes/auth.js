...
router.post('/signup', recaptcha.middleware.verify, captchaRegister, async (req, res) => {
    ...
    if (!phoneNumber /*|| !username || !password || !confirmPassword*/ ) {
        ...
        user.findOne({ $or: [{ phoneNumber: phoneNumber }, { username: username }] }, async function (err, data) {
            if (err) throw err;
            if (data) {
                ...
                try {
                    ...
                    await user.create({
                        ...
                        password: hash,
                        apikey: generateApiKey({ method: 'uuidv4' }),
                    ...
                    console.error(err);
                    req.flash('error_messages',"Something went wrong during account creation.");
                    res.redirect('/signup');
                }
            }
        });
