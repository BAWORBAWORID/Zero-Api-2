...
module.exports.sendVerifyWA = async (wa, token) => {
  return new Promise(async (resolve, reject) => {

    const url = `http://${domain}/verifyemail?token=` + token;
    const teks = `*Verify Your Account*
    *Url*: ${url}
    *Expired*: 30 *Menit*`;

    // Mengirim pesan verifikasi melalui WhatsApp
    try {
      const response = await fetch(`https://wa.frsender.my.id/send-message?api_key=uV4Klg1oLJPiT44gn4j5dkvN66AkPN&sender=6288297563383&number=${wa}&message=${teks}`);
      const resJson = await response.json();
      console.log(resJson); // Menampilkan respon dari API WhatsApp
      resolve(resJson); // Mengembalikan respon dari API WhatsApp
    } catch (error) {
      console.error(error);
      reject(error); // Mengembalikan error jika terjadi kesalahan dalam pengiriman
    }
  });
}
